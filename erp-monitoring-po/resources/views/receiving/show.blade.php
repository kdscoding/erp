...
@section('content')
        ...
        <div class="card-body">
            <div class="row g-2">
                ...
                <div class="col-md-3">
                    ...
                    <div><x-status-badge :status="$receipt->status" scope="gr" /></div>
                </div>

                <div class="col-md-3">
                    <strong>Shipment</strong>
                    <div>{{ $receipt->shipment_number ?: '-' }}</div>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <div>{{ $receipt->invoice_number ?: '-' }}</div>
                </div>
                <div class="col-md-3">
                    <strong>Tanggal Invoice</strong>
                    <div>{{ $receipt->invoice_date ? \Carbon\Carbon::parse($receipt->invoice_date)->format('d-m-Y') : '-' }}
                    </div>
                </div>

                <div class="col-md-3">
                    <strong>Currency</strong>
                    <div>{{ $receipt->invoice_currency ?: '-' }}</div>
                </div>
                <div class="col-md-3">
                    <strong>PO</strong>
                    <div>{{ $receipt->po_number }}</div>
                </div>
                <div class="col-md-3">
                ...
                @if ($receipt->cancel_reason)
                    <div class="col-md-12">
                        ...
                        <div class="text-danger">{{ $receipt->cancel_reason }}</div>
                    </div>
                @endif
            </div>
        </div>
    ...
    @if ($receipt->status === \App\Support\DocumentTermCodes::GR_POSTED)
        <div class="card card-outline card-danger mb-3">
            <div class="card-header">
                <h3 class="card-title">Batalkan Goods Receipt</h3>
            </div>
            <div class="card-body">
                <div class="alert alert-warning">
                    Gunakan hanya jika posting GR salah. Sistem akan mengembalikan qty received ke shipment dan PO terkait.
                </div>
                <form method="POST" action="{{ route('receiving.cancel', $receipt->id) }}">
                    ...
                    <button class="btn btn-danger btn-sm"
                        onclick="return confirm('Batalkan GR ini dan kembalikan qty receiving?')">
                        Batalkan GR
                    </button>
                </form>
            </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Detail Item Goods Receipt</h3>
        </div>
                ...
                <tbody>
                    @foreach ($items as $item)
                        <tr>
                            ...
                            <td>{{ $item->po_unit_price !== null ? \App\Support\NumberFormatter::trim($item->po_unit_price) : '-' }}
                            </td>
                            <td>{{ $item->invoice_unit_price !== null ? \App\Support\NumberFormatter::trim($item->invoice_unit_price) : '-' }}
                            </td>
                            <td>{{ $item->invoice_line_total !== null ? \App\Support\NumberFormatter::trim($item->invoice_line_total) : '-' }}
                            </td>
                            <td>{{ \App\Support\NumberFormatter::trim($item->shipped_qty ?? 0) }} {{ $item->unit_name }}
                            </td>
                            ...
                            <td>{{ \App\Support\NumberFormatter::trim($item->total_po_received_qty) }}
                                {{ $item->unit_name }}</td>
                            <td>{{ \App\Support\NumberFormatter::trim($item->outstanding_qty) }} {{ $item->unit_name }}
                            </td>
                            <td>{{ $item->remark ?: '-' }}</td>
                        </tr>
                    @endforeach
