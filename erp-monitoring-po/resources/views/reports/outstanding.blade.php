...
<form method="GET" class="row g-2 align-items-end">
    ...
    <div class="col-md-2"><label class="form-label">Status</label><select name="status" class="form-select"><option value="">Semua</option>@foreach(['Draft','Submitted','Approved','Sent to Supplier','Supplier Confirmed','Shipped','Partial Received','On Hold / Discrepancy'] as $status)<option value="{{ $status }}" @selected(request('status') === $status)>{{ $status }}</option>@endforeach</select></div>
