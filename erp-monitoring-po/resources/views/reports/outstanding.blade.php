...
<div class="card"><div class="card-body table-responsive p-0"><table class="table table-striped mb-0"><thead><tr><th>PO Number</th><th>Tanggal</th><th>Supplier</th><th>Status</th><th>ETA</th></tr></thead><tbody>@forelse($rows as $row)<tr><td>{{ $row->po_number }}</td><td>{{ \Carbon\Carbon::parse($row->po_date)->format('d-m-Y') }}</td><td>{{ $row->supplier_name }}</td><td>{{ \App\Support\TermCatalog::label('po_status', $row->status, $row->status) }}</td><td>{{ $row->eta_date ? \Carbon\Carbon::parse($row->eta_date)->format('d-m-Y') : '-' }}</td></tr>@empty<tr><td colspan="5" class="text-center text-muted">Tidak ada data.</td></tr>@endforelse</tbody></table></div></div>
