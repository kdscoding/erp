...
<div class="card"><div class="card-body table-responsive p-0"><table class="table table-hover text-nowrap mb-0"><thead><tr><th>No Shipment</th><th>PO</th><th>Supplier</th><th>Status</th><th>Delivery Note</th><th>Tgl</th><th>ETA</th><th>Aksi</th></tr></thead><tbody>@foreach($rows as $r)<tr><td>{{ $r->shipment_number }}</td><td>{{ $r->po_number }}</td><td>{{ $r->supplier_name }}</td><td><span class="badge {{ $r->status === 'Draft' ? 'bg-secondary' : ($r->status === 'Shipped' ? 'bg-primary' : ($r->status === 'Partial Received' ? 'bg-warning text-dark' : 'bg-success')) }}">{{ $r->status }}</span></td><td>{{ $r->delivery_note_number ?: '-' }}</td><td>{{ \Carbon\Carbon::parse($r->shipment_date)->format('d-m-Y') }}</td><td>{{ $r->eta_date ? \Carbon\Carbon::parse($r->eta_date)->format('d-m-Y') : '-' }}</td><td>@if($r->status === 'Draft')<form method="POST" action="{{ route('shipments.mark-shipped', $r->id) }}">@csrf @method('PATCH')<button class="btn btn-sm btn-outline-primary">Konfirmasi Shipped</button></form>@else - @endif</td></tr>@endforeach</tbody></table></div></div>
